"""
    sphinxnotes.anything
    ~~~~~~~~~~~~~~~~~~~~

    The package provides directives, roles, indexes and domains
    for descripting anything.
...
if True: # For type annotation
    from typing import Tuple, Dict, Optional, Any, Iterator, Type, Callable, List
...
class AnyRole(XRefRole):
    '''
    Role implementation for refering to anything. '''
...
def register(template:Template) -> None:
    ...
    '''
    if _registry.get(template.name):
        logger.warning('template %s already exists, ignored this one' % template.name)
        return
